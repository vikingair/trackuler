...
export const Bookings: React.FC<BookingsProps> = ({ extendedTracks }) => {
    ...
    const tracksByTags = Object.entries(
        taggedTracks.reduce((red, cur) => {
            ...
            const existingTrack = red[cur.tag].tracks.find(({ text }) => text === cur.text);
            if (existingTrack) {
        ...
        <details>
            ...
            {tracksByTags.map(([tag, { timeDiffMs, tracks }]) => (
                <div className="booking" key={tag}>
                    <h3>
                        <span>{tag}</span>
                        <span className="no-user-select">{TrackService.toReadableTimeDiff(timeDiffMs)}</span>
                    </h3>
                    <ul>
